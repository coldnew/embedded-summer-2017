...
*** 使用到的場合: CRC-32 運算
    ...
    從 [[*使用到的場合: 加解密運算][使用到的場合: 加解密運算]] 這邊可以知道僅僅只是反轉一下位元就可以製作加解密程式，實務上，[[https://zh.wikipedia.org/zh-tw/循環冗餘校驗][循環冗餘校驗 (CRC)]] 的實作就有用到 位元反轉 (bit reverse) 的部分。
...
*** 補充: Clang 與 GCC 支援
    ...
    Clang 是有內建 [[http://clang.llvm.org/docs/LanguageExtensions.html#builtin-bitreverse][字元反轉]] 的函式的，不過 GCC 根據 [[https://gcc.gnu.org/bugzilla/show_bug.cgi?id=50481][BUG 50481]] 來看是尚未支援內建這功能。
...
*** 實作 2 - 實作 1 改良版
    ...
    反之，透過每一個記憶體區塊的大小找尋下一組可以使用的記憶體區塊，並反回位址。
    ...
    #+BEGIN_SRC C  :tangle no :nowebref q3-malloc2-main
      int main(int argc, char *argv[])
      {
	      my_init();
...
*** 使用情境
    ...
    此題目的應用場合應該是在有限的程式空間 (flsh size 很小)的情況下，需要簡單的記憶體管理機制，使用這種方式來管理記憶體雖然性能不好，但是也可能夠用。
...
*** 思考 * 想法
    ...
    我們將 =i= 數值調小來觀看程式，並搭配 =getpid()= 去取得 process ID
    ...
    #+BEGIN_SRC C
      for (int i = 0; i < 2; i++) {
	      pid_t pid = fork();
	      if (pid == 0)
		      printf("child_pid %d round %d\n", getpid(), i);
	      else
		      printf("parend_pid %d round %d\n", getpid(), i);
      }
      wait(NULL); wait(NULL); wait(NULL);
    #+END_SRC

    #+RESULTS:
    | parend_pid | 72228 | round | 0 |
    | child_pid  | 72230 | round | 1 |
    | child_pid  | 72229 | round | 0 |
    | child_pid  | 72231 | round | 1 |
    | child_pid  | 72229 | round | 0 |
    | parend_pid | 72229 | round | 1 |
    | parend_pid | 72228 | round | 0 |
    | parend_pid | 72228 | round | 1 |
    ...
    | parend_pid | 72058 | round | 0 |
    | parend_pid | 72058 | round | 0 |
    | parend_pid | 72058 | round | 0 |
    | parend_pid | 72058 | round | 0 |
    | parend_pid | 72058 | round | 1 |
    | parend_pid | 72058 | round | 1 |
    | parend_pid | 72058 | round | 2 |

    | parend_pid | 72059 | round | 1 |
    | parend_pid | 72059 | round | 1 |
    | parend_pid | 72059 | round | 2 |
    | child_pid  | 72059 | round | 0 |
    | child_pid  | 72059 | round | 0 |
    | child_pid  | 72059 | round | 0 |
    | child_pid  | 72059 | round | 0 |

    | parend_pid | 72060 | round | 2 |
    | parend_pid | 72061 | round | 2 |

    | child_pid  | 72063 | round | 2 |
    | child_pid  | 72062 | round | 2 |
    | child_pid  | 72060 | round | 1 |
    | child_pid  | 72064 | round | 2 |
    | child_pid  | 72060 | round | 1 |

    | child_pid  | 72061 | round | 1 |
    | child_pid  | 72065 | round | 2 |

    | child_pid  | 72061 | round | 1 |


    執行第一次的時候，原先的 process 透過 =fork()= 生出一個新的 process, 兩者都會執行 =printf()= 因此第一輪共顯示 =2 次=
    ...
    #+RESULTS:
    | parend_pid | 72033 | round | 0 |
    | child_pid  | 72035 | round | 1 |
    | child_pid  | 72034 | round | 0 |
    | child_pid  | 72036 | round | 1 |
    | child_pid  | 72034 | round | 0 |
    | parend_pid | 72034 | round | 1 |
    | parend_pid | 72033 | round | 0 |
    | parend_pid | 72033 | round | 1 |

    我們知道第一輪執行 =printf()= 兩次，而第二輪跑起來的結果是 ~8 - 2 = 6 次~ (上面程式有兩次是屬於第一輪的)。

    在第一輪中，我們總共生出了兩個 process, 進入到第二輪後，這兩個 process 會透過 =fork()= 在生出兩個 process (於是到第二輪總共有四個)

    而每個 process 都會執行 =printf()= 兩次，因此會得到 2^3 = 8 個。
